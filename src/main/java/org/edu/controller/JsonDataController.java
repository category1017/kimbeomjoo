...
public class JsonDataController {
	...
	//RestAPI서버 : 오픈차트js에서 투표한 값을 저장(업데이트)
	...
	public ResponseEntity<MemberVO> androidLogin(@RequestParam("txtUsername") String user_id, @RequestParam("txtPassword") String user_pw) {
		...
		try {
			...
			//스프링시큐리티 4.x BCryptPasswordEncoder 로 암호화된 값을 비교
			BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder(10);//매개변수10은 암호화 강 약 정도를 뜻함
	...
	public ResponseEntity<List<MemberVO>> androidMember(){
		...
		pageVO.setQueryPerPageNum(1000);//1회 쿼리에서 1000허용
		try {
